        ...
        for j in i :
            while j != 0 :
                if j % 2 == 0 : 
                    ...
                    arr_in_lst.append(1)
                j = j // 2
            arr_in_lst = arr_in_lst[::-1]
            if len(arr_in_lst) < n :
                while len(arr_in_lst) < n :
                    arr_in_lst.insert(0, 0)
            arr_out_lst.append(arr_in_lst)
            arr_in_lst = []
 
    arr1 = []
    arr2 = []
    for i in range(len(arr_out_lst)) :
        if i % 2 == 0 : 
            arr1.append(arr_out_lst[i])
        else :
            arr2.append(arr_out_lst[i])

    total = []
    for i, j in zip(arr1, arr2) :
        total.append(''.join(list(map(lambda x, y : ' ' if x + y == 0 else '#', i, j))))
    return total
